...
class WithdrawMethodController extends Controller {
    ...
    public function update_withdraw(Request $request, $id) {
        ...
        $template = $request->status === 'approved' ? 'approved_withdraw' : 'rejected_withdraw';
        $status   = cache()->get('setting')?->{'sms_' . $template};
        // send sms
        if ($status) {
            $message = SMSTemplate($template, ['user_name'=>$user->name]);
