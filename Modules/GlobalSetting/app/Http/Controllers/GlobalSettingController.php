...
class GlobalSettingController extends Controller {
    ...
    function update_commission(Request $request) {
        $rules = [
            ...
            'instructor_earning_hold' => 'required|numeric',
        ];
        $messages = [
            'commission_rate.required' => __('Commission is required'),
            'commission_rate.numeric'  => __('Commission is invalid'),

            'instructor_earning_hold.required' => __('Instructor Earning Hold days is required'),
            'instructor_earning_hold.numeric'  => __('Instructor Earning Hold days is invalid'),
        ];

        $this->validate($request, $rules, $messages);

        Setting::updateOrCreate(['key' => 'commission_rate'], ['value' => $request->commission_rate]);
        Setting::updateOrCreate(['key' => 'instructor_earning_hold'], ['value' => $request->instructor_earning_hold]);
    ...
    public function update_general_setting(Request $request) {
        ...
        $request->validate([
            ...
            'live_mail_send.required' => __('Live Class send time required'),
        ]);
        ...
        Setting::where('key', 'is_refundable')->update(['value' => $request?->is_refundable]);
    ...
    public function update_logo_favicon(Request $request) {
        checkAdminHasPermissionAndThrowException('setting.update');
        $request->validate([
            'logo' => ['nullable', 'image', 'max:2048','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
            'favicon' => ['nullable', 'image', 'max:512','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
            'preloader' => ['nullable', 'image', 'max:2048','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
    ...
    public function update_video_watermark(Request $request) {
        ...
        $request->validate([
            ...
            'watermark_img' => ['nullable', 'image', 'max:2048','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
        ], [
    ...
    public function update_default_avatar(Request $request) {
        checkAdminHasPermissionAndThrowException('setting.update');
        $request->validate([
            'default_avatar' => ['nullable', 'image', 'max:2048','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
    ...
    public function update_breadcrumb(Request $request) {
        checkAdminHasPermissionAndThrowException('setting.update');
        $request->validate([
            'breadcrumb_image' => ['nullable', 'image', 'max:2048','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
    ...
    public function update_maintenance_mode(Request $request) {
        ...
        $request->validate([
            ...
            'maintenance_image' => ['nullable', 'image', 'max:2048','mimetypes:image/jpeg,image/png,image/gif,image/webp,image/svg+xml'],
        ], [
