<div class="tab-pane fade active show" id="general_tab" role="tabpanel">
    <form action="{{ route('admin.update-general-setting') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mb-0">
            <label class="d-flex align-items-center">
                <input type="hidden" value="inactive" name="is_refundable" class="custom-switch-input">
                <input type="checkbox" value="active" name="is_refundable"
                    class="custom-switch-input" {{ $setting?->is_refundable == 'active' ? 'checked' : '' }}>
                <span class="custom-switch-indicator"></span>
                <span
                    class="custom-switch-description">{{ __('Refundable') }}</span>
            </label>
        </div>
